# psf8_cv.py  (PSF 8: 인지적 복잡성 / 의사결정, 교차검증 버전)
...
def run_cross_psf8():
    ...
    print("train 크기(교차검증): 전체 데이터 =", X.shape)
    print("\n=== [PSF8] 5-Fold 교차검증 Accuracy ===")
    ...
    print("=== [PSF8] 교차검증 Classification Report ===")
    print(classification_report(y, y_pred_cv))

    acc_cv = accuracy_score(y, y_pred_cv)
    print(f"정확도(accuracy): {acc_cv * 100:.2f}%")

    # -----------------------------
    # 8. 최종 모델: 전체 데이터(X, y)로 재학습 후 특성 중요도
    # -----------------------------
    ...
    print("\n=== Feature Importances (중요도 순, 전체 데이터 학습 기준) ===")
    for idx in indices:
    ...
    pred_012 = final_model.predict(x_new)[0]   # 0,1,2

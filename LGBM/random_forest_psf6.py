# psf6.py  - PSF6(환경) 수준 예측 모델 (RandomForest 버전)

import json
import pandas as pd
from sklearn.model_selection import train_test_split
from sklearn.metrics import classification_report, accuracy_score
from sklearn.ensemble import RandomForestClassifier   # ✅ 변경: LGBM → RandomForest
import numpy as np
...
def run_psf6():
    ...
    model = RandomForestClassifier(
        ...
        random_state=42,
        n_jobs=-1,         # CPU 병렬 처리
        class_weight=None, # 불균형 심하면 'balanced'로 변경 가능
    )
    ...
    print("\n=== [PSF6][RandomForest] 분류 리포트 ===")
    print(classification_report(y_test, y_pred, digits=3))

    acc = accuracy_score(y_test, y_pred)
    print(f"\n=== [PSF6][RandomForest] 정확도(Accuracy): {acc*100:.2f}%")
    ...
    print("\n=== [PSF6][RandomForest] 예시 설문 응답에 대한 예측 ===")
